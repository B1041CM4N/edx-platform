...
class ProblemComponentEditor(ContainerBase):
    ...
    # value as '234'. Whereas, bokchoy runs with the firefox, which does
    # not allow to save the value if it has a period/dot. Clicking on save
    # button after filling '2.34' in field, does not do anything and test
    # does not go any further. So, it fails always.
    ...
    def test_user_can_select_values(self):
        ...
        # revert settings
        self.problem_editor.revert_setting()
        dropdown_value = self.problem_editor.get_field_val('Problem Weight')
